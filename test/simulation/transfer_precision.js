...
const xumFragmentsGrowth = new Stochasm({ min: -0.5, max: 2.5, seed: 'fragments.org' });

let xumFragments, rebaseAmt, inflation, preRebaseSupply, postRebaseSupply;
...
async function checkBalancesAfterOperation (users, op, chk) {
  ...
  for (u in users) {
    if (Object.prototype.hasOwnProperty.call(users, u)) {
      _bals.push(await xumFragments.balanceOf.call(users[u]));
    }
  }
  await op();
  for (u in users) {
    if (Object.prototype.hasOwnProperty.call(users, u)) {
      bals.push(await xumFragments.balanceOf.call(users[u]));
    }
  }
  chk(_bals, bals);
}
...
async function checkBalancesAfterTransfer (users, tAmt) {
  await checkBalancesAfterOperation(users, async function () {
    await xumFragments.transfer(users[1], tAmt, { from: users[0] });
...
async function exec () {
  ...
  let i = 0;
  do {
    await xumFragments.rebase(i + 1, rebaseAmt, {from: deployer});
    ...
    const tAmt = (await xumFragments.balanceOf.call(deployer));
    ...
    inflation = xumFragmentsGrowth.next().toFixed(5);
    rebaseAmt = preRebaseSupply.mul(inflation).dividedToIntegerBy(1);
  } while ((await xumFragments.totalSupply.call()).plus(rebaseAmt).lt(endSupply));
}
